...
MULTICLASS_MODEL_PATH = "efficientnetb0_run5_best.keras"
BINARY_MODEL_PATH     = "efficientnetb0_binary_run5_best.keras"
...
def verify_models():
    ...
    for path, label in [(MULTICLASS_MODEL_PATH, "Multiclass"), (BINARY_MODEL_PATH, "Binary")]:
        if not os.path.exists(path):
            msgs.append(f"❌ {label} model not found: `{path}`")
        else:
            size_mb = os.path.getsize(path) / (1024 * 1024)
            msgs.append(f"✅ {label} model found: `{path}` ({size_mb:.1f} MB)")
    return msgs
...
def load_models():
    ...
    if multiclass_model.output_shape[-1] != 11:
        raise ValueError(
            f"Multiclass model output shape is {multiclass_model.output_shape} — "
            f"expected 11 classes. Wrong model file may be loaded."
        )
    if binary_model.output_shape[-1] != 1:
        raise ValueError(
            f"Binary model output shape is {binary_model.output_shape} — "
...
if uploaded_file is not None:
    ...
    with col1:
        st.markdown(
            "**Stage 1 — Food Type**\n\n"
            "EfficientNetB0 trained on 11 food classes:\n\n"
